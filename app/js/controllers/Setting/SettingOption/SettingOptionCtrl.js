...
function settingOptionController($rootScope,$scope,apiCall,apiPath,toaster,apiResponse,validationMessage) {
        ...
        apiCall.getCall(apiPath.settingOption).then(function(response2){
            ...
            for(var arrayData=0;arrayData<responseLength;arrayData++)
            {
                if(angular.isObject(response2) || angular.isArray(response2))
                {
                    ...
                    // console.log(' response2[arrayData]', response2[arrayData]);
                    ...
                    if(response2[arrayData].settingType=="taxation")
                    {
                        taxationFlag=1;
                        $scope.insertUpdateTaxationLabel = "Update";
                        var arrayData1 = response2[arrayData];
                        $scope.enableTaxationGST = arrayData1.taxationGstStatus=="enable" ? true : false;
                        // console.log('$scope.enableTaxationGST',$scope.enableTaxationGST);
                    }
            ...
            if(taxationFlag==0)
            {
                $scope.insertUpdateTaxationLabel = "Insert";
            }
            if(workFlowFlag==0)
            {
    ...
    $scope.changeInTaxation = function(key,value){
        $scope.taxationFlag = 1;
    }
    ...
    $scope.AddUpdateLanguageSetting = function() {
        ...
        if($scope.languageFlag == 1){
            ...
            // for (var pair of languageData.entries()) {
    ...
    $scope.AddUpdateTaxationSetting = function() {
        toaster.clear();
        if($scope.taxationFlag == 1){
            var taxationData = new FormData();
            if($scope.enableTaxationGST==true)
            {
                taxationData.append('taxationGstStatus','enable');
            } else {
                taxationData.append('taxationGstStatus','disable');
            }
            if ($scope.insertUpdateTaxationLabel == "Update") {
                var apiPostPatchCall = apiCall.patchCall;
            }
            else{
                var apiPostPatchCall = apiCall.postCall;
            }
            // for (var pair of taxationData.entries()) {
            //     console.log(pair[0]+ ', ' + pair[1]); 
            // }
            apiPostPatchCall(apiPath.settingOption,taxationData).then(function(response){
                if(apiResponse.ok == response){
                    
                    $scope.getOptionSettingData();
                    toaster.pop('success','Taxation-Data',$scope.insertUpdateTaxationLabel+' Successfull');
                    $scope.taxationFlag = 0;
                }
                else{
                    toaster.pop('warning','Opps!!',response);
                }
            ...
            toaster.pop('info','taxation-Data','Please Change Data');
        }
    }
