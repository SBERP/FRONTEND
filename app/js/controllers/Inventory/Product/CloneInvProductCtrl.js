...
App.controller('CloneInvProductController', CloneInvProductController);

function CloneInvProductController($scope,$rootScope,toaster,$filter,apiCall,apiPath,$stateParams,$state,apiResponse,validationMessage,getSetFactory,$modal,productFactory,fetchArrayService,maxImageSize) {
    'use strict';
    ...
    if(Object.keys(getSetFactory.get()).length > 0)
    {
        ...
        apiCall.getCall(getAllBranch).then(function(response4){
            ...
            formdata.set.branch = fetchArrayService.getfilteredSingleObject(response4,editProductData.branchId,'branchId');
        });
        ...
        formdata.set.higherPurchasePrice = editProductData.higherPurchasePrice;
        formdata.set.highestPurchasePrice = editProductData.highestPurchasePrice;
        formdata.set.mediumPurchasePrice = editProductData.mediumPurchasePrice;
        formdata.set.mediumLowerPurchasePrice = editProductData.mediumLowerPurchasePrice;
        formdata.set.lowerPurchasePrice = editProductData.lowerPurchasePrice;
        formdata.set.purchasePrice = editProductData.purchasePrice;
        ...
        formdata.set.wholesaleMarginFlat = editProductData.wholesaleMarginFlat;
        formdata.set.wholesaleMargin = editProductData.wholesaleMargin;
        formdata.set.semiWholesaleMargin = editProductData.semiWholesaleMargin;
        formdata.set.vat = editProductData.vat;
        formdata.set.mrp = editProductData.mrp;
        formdata.set.additionalTax = editProductData.additionalTax;
        formdata.set.marginFlat = editProductData.marginFlat;
        formdata.set.margin = editProductData.margin;
        ...
        if(editProductData.hsn == null){
            ...
            formdata.set.hsn = editProductData.hsn;
        }
        if(editProductData.igst == null){
            ...
            formdata.set.igst = editProductData.igst;
        }
    ...
    this.functionOption = {
        ...
        $scope.pop = function(form)
        {
            ...
            else 
            {
                if ($scope.addInvProduct.getSetProductId)
                {
                    ...
                    formdata.set('companyId',  $scope.addInvProduct.company.companyId);
                    formdata.set('productName',  $scope.addInvProduct.name);
                    formdata.set('branchId',  $scope.addInvProduct.branch.branchId);
                    formdata.set('name', $scope.addInvProduct.name);
                    formdata.set('altProductName',  $scope.addInvProduct.altProductName);
                    formdata.set('productDescription',  $scope.addInvProduct.productDescription);
                    formdata.set('color',  $scope.addInvProduct.color); 
                    formdata.set('size',  $scope.addInvProduct.size);
                    formdata.set('variant',  $scope.addInvProduct.variant);
                    formdata.set('barcodeNo',  $scope.addInvProduct.productCode);
                    formdata.set('highestUnitQty',  $scope.addInvProduct.highestUnitQty);
                    formdata.set('higherUnitQty',  $scope.addInvProduct.higherUnitQty);
                    formdata.set('lowestUnitQty',  $scope.addInvProduct.lowestUnitQty);
                    formdata.set('lowerUnitQty',  $scope.addInvProduct.lowerUnitQty);
                    formdata.set('mediumUnitQty',  $scope.addInvProduct.mediumUnitQty);
                    formdata.set('mediumLowerUnitQty',  $scope.addInvProduct.mediumLowerUnitQty);
                    
                    formdata.set('highestMouConv',  $scope.addInvProduct.highestMouConv);
                    formdata.set('higherMouConv',  $scope.addInvProduct.higherMouConv);
                    formdata.set('lowestMouConv',  $scope.addInvProduct.lowestMouConv);
                    formdata.set('lowerMouConv',  $scope.addInvProduct.lowerMouConv);
                    formdata.set('mediumMouConv',  $scope.addInvProduct.mediumMouConv);
                    formdata.set('mediumLowerMouConv',  $scope.addInvProduct.mediumLowerMouConv);
                    
                    formdata.set ('wholesaleMarginFlat',  $scope.addInvProduct.wholesaleMarginFlat);
                    formdata.set ('wholesaleMargin',  $scope.addInvProduct.wholesaleMargin);
                    formdata.set ('vat',  $scope.addInvProduct.vat);
                    formdata.set ('mrp',  $scope.addInvProduct.mrp);
                    formdata.set ('additionalTax',  $scope.addInvProduct.additionalTax);
                    formdata.set ('marginFlat',  $scope.addInvProduct.marginFlat);
                    formdata.set ('margin',  $scope.addInvProduct.margin);
                    ...
                    formdata.set('productCategoryId',$scope.addInvProduct.category.productCategoryId);
                    formdata.set('productGroupId',$scope.addInvProduct.group.productGroupId);
                    formdata.set('measurementUnit',$scope.addInvProduct.measureUnit.measurementUnitId);
                    formdata.set('itemCode', $scope.addInvProduct.itemCode);
                    formdata.set('purchasePrice',  $scope.addInvProduct.purchasePrice);
                    formdata.set('hsn',  $scope.addInvProduct.hsn);
                    
                    // if($scope.enableDisableWebIntegration == true){
                    //     formdata.set('webintegration','on');
                    //     $scope.uploadFile();
                    //     $scope.uploadMultipleFile();
                    // } else{
                    //     formdata.set('webintegration','off');
                    // }
                    
                    formdata.set('igst',  $scope.addInvProduct.igst);
                    formdata.set('minimumStockLevel',  $scope.addInvProduct.minimumStockLevel);
                    formdata.set('opening',  $scope.addInvProduct.opening);
                    formdata.set('maxSaleQty',  $scope.addInvProduct.maxSaleQty);
                    formdata.set('remark',  $scope.addInvProduct.remark);
                    // formdata.set('quantityWisePricing[0][productId]','') ;
                    ...
                    apiCall.postCall(apiPath.getAllProduct,formdata).then(function(response5) {
                        // console.log('api response5',response5);
                        toaster.clear();
                        ...
                        if (apiResponse.ok == response5) {
                            ...
                            // console.log('productFactory.setNewProduct',productFactory.setNewProduct);
        ...
        $scope.uploadFile = function(files) {
            // console.log('function called for files');
    ...
    CloneInvProductController.$inject = ["$scope","$rootScope","toaster","$filter","apiCall","apiPath","$stateParams","$state","apiResponse","validationMessage","getSetFactory","$modal","productFactory","fetchArrayService","maxImageSize"];