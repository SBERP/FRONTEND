...
App.controller('monthlyreportctrl',monthlyreportctrl);

function monthlyreportctrl($scope, $modalInstance,$rootScope, $filter, ngTableParams,$http,$sce,$window,apiCall,apiPath,flotOptions, colors,$timeout,$state,responseData,toaster,getSetFactory,draftOrSalesOrder,apiResponse) {
    // console.log('in controller',$modalInstance);
    'use strict';
    ...
    **/
    ...
    $scope.editDataViewspurchase = function(id){
        // console.log(id);
        
        apiCall.getCall(apiPath.getpurchasebillbyid+id).then(function(response){
            // console.log('purchasedata',response);
    ...
    $scope.setLength = function(userProductArray){
        // console.log('userProductArray',userProductArray);
...
monthlyreportctrl.$inject = ["$scope", "$modalInstance","$rootScope", "$filter", "ngTableParams","$http","$sce","$window","apiCall","apiPath","flotOptions","colors","$timeout","$state","responseData","toaster","getSetFactory","draftOrSalesOrder","apiResponse"];
