...
function SalesReportController($rootScope,$scope, $filter, $http, ngTableParams,apiCall,apiPath,flotOptions, colors,$timeout,getSetFactory,$state,headerType,$modal,$window,toaster,apiResponse,apiDateFormate, productFactory) {
    ...
    if($scope.headerType == 'Wholesales' && $scope.headerType!=='purchase'){
        ...
        apiCall.getCall(apiPath.getAllSalebill).then(function(response) {
        ...
        $scope.openBillHistoryModal = function (size,responseData,draftOrSalesOrder) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'app/views/PopupModal/QuickMenu/monthlysalesmodal.html',
        ...
        $scope.openBillHistoryModalpurchase = function (size,responseData,draftOrSalesOrder) {
            ...
            toaster.pop('wait', 'Please Wait', 'Modal Data Loading....',60000);
            var modalInstance = $modal.open({
                templateUrl: 'app/views/PopupModal/QuickMenu/monthlypurchasemodal.html',
                controller: monthlyreportctrl,
                size: size,
                resolve:{
                    responseData: function(){
...
SalesReportController.$inject = ["$rootScope","$scope", "$filter","$http", "ngTableParams","apiCall","apiPath","flotOptions","colors","$timeout","getSetFactory","$state","headerType","$modal","$window","toaster","apiResponse","apiDateFormate","productFactory"];