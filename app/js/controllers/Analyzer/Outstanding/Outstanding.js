...
App.controller('OutstandingController', OutstandingController);

function OutstandingController($rootScope,$scope, $filter, $http, ngTableParams,apiCall,apiPath,flotOptions, colors,$timeout,getSetFactory,$state,headerType,$modal,$window,toaster,apiResponse,apiDateFormate, productFactory) {
    ...
    if($scope.headerType == 'Wholesales' && $scope.headerType!=='purchase'){
        // console.log('headerType',$scope.headerType);
        apiCall.getCall(apiPath.getAllOutstanding).then(function(response) {
            // console.log('response from 1st apicall',response);
            data = response.receivable;
            $scope.users=data;
            // console.log('users',$scope.users);
            // console.log('users',$scope.users);
        })
        // apiCall.getCall(apiPath.getAllOutstanding).then(function(resoutstanding){
        //     console.log('resoutstanding',resoutstanding);
        // });
        
        $scope.myfunction =function(user){
            data = user.salesBills;
            // console.log(user);
            if(angular.isArray(data)){
                // if(data.length > 1)
                // console.log(data,'data');
                $scope.openBillHistoryModal('lg',data);
            }
        }
        $scope.openBillHistoryModal = function (size,responseData,draftOrSalesOrder) {
            toaster.clear();
            // console.log('inside function response',responseData);
            if (Modalopened) return;
            
            toaster.pop('wait', 'Please Wait', 'Modal Data Loading....',60000);
            var modalInstance = $modal.open({
                templateUrl: 'app/views/PopupModal/QuickMenu/outstanding_receivable.html',
                controller: monthlyreportctrl,
                size: size,
                resolve:{
                    responseData: function(){
                        // console.log('after html call responseData',responseData);
                        return responseData;
                    },
                    draftOrSalesOrder: function(){
                        // console.log('after html call draftOrSalesOrder',draftOrSalesOrder);
        ...
        apiCall.getCall(apiPath.getAllOutstanding).then(function(response) {
            ...
            data2 = response.payable;
            $scope.users=data2;
            // console.log('users',$scope.users);
        })
        $scope.myfunctionpurchase =function(user){
            // console.log('user',user);
            data2 = user.purchaseBills;
            // console.log('data2',data2);
            if(angular.isArray(data2)){
                // if(data2.length > 1)
                // console.log(data2,'data2');
                $scope.openBillHistoryModalpurchase('lg',data2);
            }
        }
        $scope.openBillHistoryModalpurchase = function (size,responseData,draftOrSalesOrder) {
            ...
            toaster.pop('wait', 'Please Wait', 'Modal Data Loading....',60000);
            var modalInstance = $modal.open({
                templateUrl: 'app/views/PopupModal/QuickMenu/outstanding_payable.html',
                controller: monthlyreportctrl,
                size: size,
                resolve:{
                    responseData: function(){
    ...
    $scope.loadInit = function(onDateChange = null) 
    {
        ...
        // console.log('getJrnlPath',getJrnlPath);
    ...
    $scope.columnHideShow = [
        ...
        {
            // console.log('$scope.billData',$scope.billData);
            var tempObject = {};
        ...
        {
            // console.log('reLoadPdfData',response);
            toaster.clear();
...
OutstandingController.$inject = ["$rootScope","$scope", "$filter","$http", "ngTableParams","apiCall","apiPath","flotOptions","colors","$timeout","getSetFactory","$state","headerType","$modal","$window","toaster","apiResponse","apiDateFormate","productFactory"];