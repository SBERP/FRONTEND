...
App.controller('CategoryReportController', CategoryReportController);

function CategoryReportController($rootScope,$scope, $filter, $http, ngTableParams,apiCall,apiPath,flotOptions, colors,$timeout,getSetFactory,$state,headerType,$modal,$window,toaster,apiResponse,apiDateFormate, productFactory) {
    ...
    if($scope.headerType == 'Categorywise'){
        ...
        $scope.openBillHistoryModal = function (size,responseData,draftOrSalesOrder) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'app/views/PopupModal/QuickMenu/categorywisereport.html',
...
CategoryReportController.$inject = ["$rootScope","$scope", "$filter","$http", "ngTableParams","apiCall","apiPath","flotOptions","colors","$timeout","getSetFactory","$state","headerType","$modal","$window","toaster","apiResponse","apiDateFormate","productFactory"];