...
App.controller('AccPurchaseDetailTaxationController', AccPurchaseDetailTaxationController);

function AccPurchaseDetailTaxationController($rootScope,$scope, $filter, ngTableParams,apiCall,apiPath,apiResponse,toaster,getSetFactory,$window) {
  'use strict';
  var vm = this;
	//$scope.brandradio="";
  ...
  var data = [];
	...
	$scope.getProduct = function(id){
		...
		apiCall.getCall(apiPath.getPurchaseDetail+id).then(function(response){
  ...
  /*** Pdf ***/
		...
		$scope.generatePdf = function(operation){
			...
			apiCall.getCallHeader(apiPath.getPurchaseDetail+$scope.stateCheck.companyId,getData).then(function(responseDrop){
...
AccPurchaseDetailTaxationController.$inject = ["$rootScope","$scope", "$filter", "ngTableParams","apiCall","apiPath","apiResponse","toaster","getSetFactory","$window"];